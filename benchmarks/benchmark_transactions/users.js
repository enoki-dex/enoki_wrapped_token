...
class User {
  ...
  async init() {
    await command(`dfx identity new ${this.name} || true`);
    this.principal = await command(`dfx --identity ${this.name} identity get-principal`);
  }
  ...
  async update_balance() {
    let response = await command(`dfx canister call xtc_token balanceOf '(principal "${this.principal}")'`, () => true);
    this.balance = parse_candid(response)[0][0];
  }

  async update_shard_balance() {
    let response = await command(`dfx canister call ${this.assigned_shard} shardBalanceOf '(principal "${this.shard_principal}")'`, () => true);
    this.balance = parse_candid(response)[0][0];
  }

  async fund() {
    let old_balance = this.balance;
    await command(`dfx canister call xtc_token transfer '(principal "${this.principal}",${FUNDING_AMOUNT_PER_USER})'`, async () => {
      await this.update_balance();
      return this.balance === old_balance;
    });
    await this.update_balance();
  ...
  async fund_wrapped() {
    let old_balance = this.balance;
    await command(`dfx canister call ${default_assigned_shard} shardTransfer '(principal "${this.assigned_shard}", principal "${this.shard_principal}",${FUNDING_AMOUNT_PER_USER})'`, async () => {
...
const prepare_fund_users = async () => {
  await command('dfx identity use default');
};
...
export const init_wrapped = async () => {
  await command('dfx identity use default');
  default_assigned_shard = parse_candid(await command(`dfx canister call enoki_wrapped_token startRegistration`))[0][0];
  console.log(`assigned: ${default_assigned_shard}`);
  default_shard_principal = parse_candid(await command(`dfx canister call ${default_assigned_shard} whoami`))[0][0];
  console.log(`me: ${default_shard_principal}`);
  await command(`dfx canister call enoki_wrapped_token completeRegistration '(principal "${default_shard_principal}")'`);
...
export const fund_users_wrapped = async () => {
  await command(`dfx canister call xtc_token approve '(principal "${default_assigned_shard}", ${FUNDING_AMOUNT_TOTAL})'`);
  await command(`dfx canister call ${default_assigned_shard} wrap "(${FUNDING_AMOUNT_TOTAL})"`);
